      ...
      <div className="w-[80%] mx-auto mt-20 mb-20 flex gap-6">
        ...
        >
          <div className="relative w-16 h-6">
            {/* Slider bar (below) */}
            <div
              className={`absolute top-1/2 left-0 w-12 h-2 -translate-y-1/2 rounded-2xl transition-all duration-300 ease-in-out
                                ${selected === "veg" ? "bg-green-500" : "bg-white"}`}
            ></div>

            {/* Slider knob with icon */}
            <div
              className={`absolute w-6 h-6 bg-white rounded-full shadow-md flex items-center justify-center transition-all duration-300 ease-in-out
