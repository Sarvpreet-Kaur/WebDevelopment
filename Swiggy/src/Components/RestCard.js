...
export default function RestCard({restInfo}){
    ...
    return (
        <div className="max-w-70 mv-2 transform transition duration-200 hover:scale-95 mb-7">
            <Link to={`/city/delhi/${restInfo?.info?.id}`}>
                <img className="w-70 h-45 object-cover rounded-xl" src={`https://media-assets.swiggy.com/swiggy/image/upload/${restInfo?.info?.cloudinaryImageId}`} />
                ...
                <div className="w-[95%] mx-auto mt-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-lg font-semibold">{(restInfo?.info?.sla?.slaString == undefined)?"": restInfo?.info?.sla?.slaString}</span>
                    </div>
